...
class AccountPlanController extends Controller
{
    ...
    public function generate(Request $request)
    {
        ...
        try {
            ...
            foreach ($taxCodesData as $taxData) {
                ...
                // Finde das zugehörige Steuerkonto (Zuordnung lebt im Generator, siehe
                // Skr03AccountPlanGenerator::defaultAccountCodeForTaxCode()).
                $accountId = null;
                $accountCode = $this->generator->defaultAccountCodeForTaxCode($taxData['code']);
                if ($accountCode) {
                    $accountId = Account::where('code', $accountCode)->first()?->id;
                }
    ...
    public function missing(Request $request)
    {
        ...
        $missingAccounts = array_filter($accountsData, fn ($acc) => ! in_array($acc['code'], $existingCodes)
        );
